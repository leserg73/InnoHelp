...
function toggle_node(id)
{
  ...
  objContent.style.display = expanding ? "" : "none";
    ...
    const img = document.getElementById("nodeimg_" + id);
    img.dataset.themeImage = expanding
        ? "../images/contentsheadopen.svg"
        : "../images/contentsheadclosed.svg";

    // Обновляем изображение через ThemeManager
    const currentTheme = document.documentElement.getAttribute('data-theme') || 'light';
    ThemeManager.updateThemeImages(currentTheme);
...
function init_index_tab()
{
  ...
  script.src = "../js/contentsindex.js";
...
function init_index_tab_elements()
{
  ...
  if (contentsIndexData) {
    ...
    for (i = 0; i < len; i++) {
      ...
      if (!matches) break;
      htmlArray[i] = '<a href="../htm/topic_' + matches[1] + ".htm" +
